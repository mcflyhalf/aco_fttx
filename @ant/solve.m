...
function s = solve(ant,wld) % abcd needs to be an instance of world
  ...
  s.Lhc = ant.Lhc_matrix %zeros(closures by homes)
  s.Lcs = ant.Lcs_matrix % zeros(splitters by closures
  s.DistHC_matrix =world.DistHC_matrix
  s.DistCS_matrix = world.DistCS_matrix
  s.Pher_HC = world.Pher_HC;
  s.Pher_CS = world.Pher_CS;
  s.home_no = world.home_no
  s.closure_no = world.closure_no
  s.splitter_no = world.splitter_no
  MaxLinks_CH = 2 % should be pulled from world
  MaxLinks_SC = 3 % should be pulled from world
  MaxDistance_HC = 350 % should be pulled from world
  MaxDistance_CS = 300 % should be pulled from world
 ...
 % Trial 2: With pheromone matrices: stochastic
   for h = 1:world.home_no % for all homes, starting with home 1
    ...
    for c= 1:closure_no
      used_cl = used_elements(Lhc) % returns an array of 1/0 values 
