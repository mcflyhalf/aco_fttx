function c= cost(ant,wld,solution)
  ...
  Cost_per_cl = 8000;
  Cost_per_sp = 15500;
  Drop_cable_cost_per_m = 62;
  Distr_cable_cost_per_m = 80;
  ...
  c.dropc_cost = sum(sum(world.DistHC_matrix .* solution.Lhc * Drop_cable_cost_per_m));
  c.distric_cost = sum(sum(world.DistCS_matrix .* solution.Lcs * Distr_cable_cost_per_m));
...
##  if the solution is invalid, give it an infinite cost
