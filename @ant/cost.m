function c= cost(ant,wld,solution)
  ...
  c.closure_cost = sum(sum(solution.Used_cl),2)*Cost_per_cl; %should it be ant.Used_cl or used_elements.Used.cl???
  c.splitter_cost = sum(sum(solution.Used_sp),2)*Cost_per_sp; % finds total number of used splitters using sum() then computes cost
  c.dropc_cost = (world.DistHC_matrix'(:)')*(solution.Lhc'(:))*Drop_cable_cost;
  c.distric_cost = (world.DistCS_matrix'(:)')*(solution.Lcs'(:))*Distr_cable_cost;
